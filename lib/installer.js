/**
 * dsh-git-rescue 2.1.0 — 救援环境安装器（2026-08-21 用户需求）
 *
 * 主环境只装一个「小插件/入口」，专职：把最新救援插件源码安装到救援环境
 * （测试环境 + 纯净环境）。主环境本身不装完整救援插件
 * （避免迭代代码写死主环境），guardian 独立守护。
 *
 * 救援环境命名（2026-08-21 用户确认：代码写死重构版新命名，统一 @Save-*）：
 *   <dsh版本>@Save-clean   纯净环境（初始、无插件、带防装插件锁定）
 *   <dsh版本>@Save-test    测试环境（开发者随便折腾）
 * 命名规则由 lib/rescue-env.js 的 rescueEnvName 生成，本模块不再使用旧
 * dsh-test-home 等历史命名。
 *
...
import { rescueEnvName, ensureRescueEnvRoot, selfPluginDir } from './rescue-env.js'
...
/** 测试环境目录：<dsh版本>@Save-test（重构版新命名，代码写死规则）。 */
export function testEnvDir(dshVersion) {
  return join(cleanEnvRoot(), rescueEnvName(dshVersion, 'test'))
}

/** 测试环境开发母体插件目录。 */
export function testEnvPluginDir(dshVersion) {
  return join(testEnvDir(dshVersion), 'profiles/web/node_modules_local', TEST_ENV.name)
}

/** 纯净环境根（与 rescue-env.js 的 rescueEnvRoot 一致：插件项目内部 rescue-envs/，不存在自动创建）。 */
export function cleanEnvRoot() {
  const projectDir = process.env.DSH_GIT_RESCUE_PROJECT_DIR
  if (projectDir) return join(projectDir, 'rescue-envs')
  return join(selfPluginDir(), 'rescue-envs')
}

/** 确保纯净环境根目录存在（不存在则创建）。 */
export async function ensureCleanEnvRoot() {
  const root = cleanEnvRoot()
  await fs.mkdir(root, { recursive: true })
  return root
}

/** 纯净环境目录：<dsh版本>@Save-clean（重构版新命名，代码写死规则）。 */
export function cleanEnvDir(dshVersion) {
  return join(cleanEnvRoot(), rescueEnvName(dshVersion, 'clean'))
}
...
/** 安装源码到测试环境（三要素：源码复制 + patch 注册 + 软链）。测试环境 = <dsh版本>@Save-test。 */
export async function installToTestEnv(tmpDir, dshVersion, { force = false } = {}) {
  const home = testEnvDir(dshVersion)
  const target = join(home, 'profiles/web/node_modules_local', TEST_ENV.name)
  try {
    // 测试环境不存在则自动创建骨架（2026-08-21 用户要求：没有则救援插件创建）
    await ensureCleanEnvRoot()
    await fs.mkdir(join(home, 'profiles/web/node_modules_local'), { recursive: true })
    await fs.mkdir(join(home, 'sessions'), { recursive: true })
    await fs.mkdir(join(home, 'storages'), { recursive: true })
    await fs.mkdir(join(home, 'skills'), { recursive: true })
    const patchPath = join(home, 'profiles/web/cordis.patch.yml')
    if (!(await fs.access(patchPath).then(() => true).catch(() => false))) {
      await fs.writeFile(patchPath, '# dsh profile root — an empty entry list. The tree is composed as patches:\n', 'utf8')
    }
    // patch 注册（不存在才加）
    ...
    // 软链
    await fs.mkdir(join(home, 'profiles/web/node_modules'), { recursive: true })
    await fs.symlink(`../node_modules_local/${TEST_ENV.name}`, join(home, 'profiles/web/node_modules', TEST_ENV.name)).catch(() => {})
    return { ok: true, target, created: true }
  } catch (e) {
 ...
 */
async function resolveSource(tmpDir, dshVersion) {
  // 本地测试环境副本（开发母体）
  const localDir = testEnvPluginDir(dshVersion)
...
export async function installerStatus(dshVersion = '0.1.0-rc.6') {
  const remote = await checkRemoteVersion()
  const localPkg = await envInstalledVersion(testEnvPluginDir(dshVersion))
  const testVer = await envInstalledVersion(join(testEnvDir(dshVersion), 'profiles/web/node_modules_local', TEST_ENV.name))
  ...
  return {
    ...
    testEnv: { version: testVer, dir: testEnvDir(dshVersion) },
