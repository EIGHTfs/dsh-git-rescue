/**
 * dsh-git-rescue 2.0.0 — 零依赖 zip 工具（合并自旧版 dsh-snapshot-archive）
 *
 * Store 模式 ZIP 生成/解析，跨平台（Linux/macOS/Windows），不依赖系统 zip 命令。
 * 实现 ZIP 规范：
 ...
 *
 * 用途（需求5 快照归档还原点）：把救援过程中 git diff 的改动文件打包成
 * 可直接应用的还原点压缩包，放到原 .dsh 根目录。
 */
