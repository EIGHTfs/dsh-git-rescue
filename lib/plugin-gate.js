 ...
 * @returns {Promise<{copied: string[], ok: boolean}>}
 */
export async function copyPluginSkills(pluginDir, dshHome) {
  const copied = []
  try {
    ...
    for (const f of entries) {
      ...
      await fs.copyFile(join(skillsDir, f), join(targetDir, f))
      copied.push(f)
    }
    return { ok: true, copied }
  } catch (e) {
    return { ok: false, copied, error: String(e?.message ?? e) }
  }
}
