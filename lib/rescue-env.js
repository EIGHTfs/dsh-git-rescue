...
/** 当前运行插件自身的源码目录（lib/ 的上级）——无论插件装在哪，创建纯净环境时把自己带进去。 */
export function selfPluginDir() {
  return dirname(dirname(fileURLToPath(import.meta.url))) // .../dsh-git-rescue
}
...
/**
 * 环境目录根：插件项目内部 <dsh-git-rescue>/rescue-envs/
 * （2026-08-21 用户决定：救援环境目录固定在插件项目内部，不随 DSH_HOME 漂移；
 *  目录不存在时由救援插件自动创建，见 ensureRescueEnvRoot）
 *
 * 解析顺序：
 *  1. DSH_GIT_RESCUE_PROJECT_DIR 环境变量（部署时权威指定项目目录，guardian/主环境均可设置）
 *  2. selfPluginDir()（插件源码目录；guardian 与安装器从项目目录运行时为项目内部）
 */
...
/** 确保救援环境根目录存在（不存在则创建），供各入口调用。 */
export async function ensureRescueEnvRoot() {
...
export async function createCleanEnv(dshVersion, { force = false, rescueSrc = null } = {}) {
  const root = await ensureRescueEnvRoot()
  ...
  try {
    ...
    // 注册唯一插件 = 救援插件（复制源码 + 依赖声明 + node_modules 软链 三要素，2026-08-21 补全）
    if (rescueSrc) {
      ...
      // package.json：声明 file: 依赖（缺此 → ERR_MODULE_NOT_FOUND）
      await fs.writeFile(
        join(dir, 'profiles/web/package.json'),
        JSON.stringify({
          name: 'dsh-profile-web',
          private: true,
          dependencies: { [pkgName]: 'file:./node_modules_local/dsh-git-rescue' },
          dsh: { profile: { bundles: ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app'] } },
        }, null, 2) + '\n',
        'utf8',
      )
      // node_modules/dsh-git-rescue 软链（Node 模块解析向上查找）
      await fs.mkdir(join(dir, 'profiles/web/node_modules'), { recursive: true })
      await fs.symlink('../node_modules_local/dsh-git-rescue', join(dir, 'profiles/web/node_modules', pkgName))
...
export async function startRescueEnv(kind, dshVersion, { appDir = '/vol1/@appcenter/deepseek-harness', force = false } = {}) {
  ...
  try {
    ...
    if (kind === 'clean' && !(await fs.access(dir).then(() => true).catch(() => false))) {
      // 2026-08-21 修复：rescueSrc 指向当前运行插件自身（原写死 join(root,'dsh-git-rescue') 导致纯净环境无插件代码）
      const created = await createCleanEnv(dshVersion, { force, rescueSrc: selfPluginDir() })
    ...
    // 随机分配端口（3083-3182，2026-08-21 用户决定：救援环境端口不固定、随机，避免与测试环境/其他实例固定端口占用冲突）
    // 每次启动重新随机，已占用端口自动跳过（ss 探测），最多尝试 40 次
    let port = 0
    ...
    for (const m of String(ssOut).matchAll(/:(\d{4,5})\s/g)) usedPorts.add(Number(m[1]))
    for (let attempt = 0; attempt < 40; attempt++) {
      const candidate = 3083 + Math.floor(Math.random() * 100) // 3083-3182
      if (!usedPorts.has(candidate)) { port = candidate; break }
    }
    if (!port) return { ok: false, error: '无空闲端口（3083-3182 随机 40 次全被占用）' }
