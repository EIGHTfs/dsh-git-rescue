...
/** 回退 .dsh 仓库到指定 ref：先 commit 现场 + 打 bad 标记，再 reset --hard。 */
async function rollbackRepo(ref, scoreType = 'crash') {
  ...
  const r = await hardReset(DSH_ROOT, ref)
  if (!r.ok) return r
  await appendEvent('rollback', { repo: 'dsh', from: brokenHead, to: ref, scoreType })
...
async function handleApi(req, res, url, method) {
  ...
  if (method === 'POST' && path === '/api/git-rescue/commit') {
    ...
    return send(res, 200, { ok: true, dsh: r.dsh })
  }
