...
// 版本号单一真源：package.json（2026-08-21 根治硬编码错版）
import { createRequire } from 'node:module'
const require = createRequire(import.meta.url)
const PKG = require('../package.json')
const PLUGIN_VERSION = PKG.version || 'unknown'
...
/** 读系统负载快照（v2.3.1）：可用内存 + loadavg + 顶部高 CPU 进程。失败静默降级。 */
async function readSystemLoad() {
  try {
    const mem = await fs.readFile('/proc/meminfo', 'utf8')
    const freeMb = (() => {
      const m = mem.match(/^MemAvailable:\s+(\d+)\s*kB/im)
      if (m) return Math.round(Number(m[1]) / 1024)
      const free = mem.match(/^MemFree:\s+(\d+)\s*kB/im)
      return free ? Math.round(Number(free[1]) / 1024) : null
    })()
    const loadavg = await fs.readFile('/proc/loadavg', 'utf8').then((s) => s.trim().split(' ').slice(0, 3).map(Number)).catch(() => null)
    return { freeMb, loadavg, at: new Date().toISOString() }
  } catch {
    return { freeMb: null, loadavg: null, at: null }
  }
}

async function collectStatus() {
  const load = await readSystemLoad()
  ...
  return {
    ...
    load, // v2.3.1：系统负载快照（可用内存 MB / loadavg / 时间）
  }
}
...
export async function apply(ctx) {
  ...
  if (tools) {
    tools.register(defineToolSimple('git_rescue_status', '查看 git-rescue 状态（git/仓库/设备/备份仓/救援环境/开机自启/自动更新）', async () => {
      ...
      return `git: ${g.version || '不可用'}${g.httpsHelperMissing ? ' (https 助手缺失→推送走 REST API)' : ''}\n` +
        ...
        (s.load?.freeMb != null ? `系统内存: 可用 ${s.load.freeMb}MB${s.load.loadavg ? `, 负载 ${s.load.loadavg.join('/')}` : ''}${s.load.freeMb < 512 ? ' ⚠️ 内存低' : ''}\n` : '') +
        auLine
    }))
    ...
    // unzip 还原点小功能（2026-08-21）：列表 / 手动构建 / 手动覆盖恢复
    tools.register(defineToolSimple('git_rescue_restorepoints', '列出本地还原点 zip（git-rescue/restore-points/，unzip 手动覆盖恢复）', async (args) => {
      const { listRestorePoints } = await import('./restore-point.js')
      const r = await listRestorePoints(DSH_ROOT)
      if (!r.ok) return `列表失败: ${r.error}`
      if (!r.points.length) return '暂无还原点（提交后自动生成）'
      return r.points.map((p) => `${p.name}  ${p.size}B  ${new Date(p.mtimeMs).toLocaleString()}`).join('\n')
    }, { type: 'object', properties: {} }))
    tools.register(defineToolSimple('git_rescue_restorepoint_build', '手动打包一次还原点（不触发 commit）', async (args) => {
      const { buildRestorePoint } = await import('./restore-point.js')
      const r = await buildRestorePoint({ dshRoot: DSH_ROOT, reason: args?.reason || 'manual-build' })
      if (!r.ok) return `构建失败: ${r.error}`
      if (r.empty) return '无未提交的 profile 变更，跳过打包'
      return `已打包还原点: ${r.name}（${r.count} 个文件，插件: ${r.plugin || 'unknown'}）`
    }, { type: 'object', properties: { reason: { type: 'string', description: '构建原因，写入 manifest' } } }))
    tools.register(defineToolSimple('git_rescue_restorepoint_restore', '从还原点 zip 手动覆盖恢复（按文件名），zip 内路径保留原始相对路径', async (args) => {
      const { restoreRestorePoint } = await import('./restore-point.js')
      const r = await restoreRestorePoint({ dshRoot: DSH_ROOT, name: args?.name })
      if (!r.ok) return `恢复失败: ${r.error}`
      return `已恢复 ${r.restored.length} 个文件：${r.restored.join(', ')}${r.skipped?.length ? `\n跳过 ${r.skipped.length} 个不安全路径: ${r.skipped.map((s) => s.name).join(', ')}` : ''}`
    }, { type: 'object', properties: { name: { type: 'string', description: '还原点 zip 文件名（含 .zip 后缀）' } } }))
    tools.register(defineToolSimple('git_rescue_restorepoint_remove', '删除指定还原点 zip', async (args) => {
      const { removeRestorePoint } = await import('./restore-point.js')
      const r = await removeRestorePoint({ dshRoot: DSH_ROOT, name: args?.name })
      return r.ok ? `已删除 ${args?.name}` : `删除失败: ${r.error}`
    }, { type: 'object', properties: { name: { type: 'string', description: '还原点 zip 文件名（含 .zip 后缀）' } } }))
  ...
  const backup = await defaultBackupRepo(STATE_ROOT)
  console.log(`[git-rescue] 已启动 v2.0.0: git=${gv || '不可用'}, dshRoot=${DSH_ROOT}, 备份仓=${backup.repo}, autoUpdate=${AUTO_UPDATE_ENABLED ? 'ON(强制)' : 'OFF(env)'}`)
}
