...
/** 远端认证配置状态（②：守护进程管理 token）。
 * 2026-08-21 用户决定：删除 SSH key 方案，仅 token。
 * 只报"是否已配置"，绝不显示明文（token 脱敏）。
 */
async function authStatus() {
  ...
  return {
    ...
    sshKeyAvailable: false, // SSH 方案已废弃（2026-08-21）
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (path === '/api/backup-select' && req.method === 'GET') {
        // 读取当前选择配置（profile 恒 true）
        const { readSelectConfig, CATEGORIES } = await import('../lib/backup-select.js')
        const cfg = await readSelectConfig(CFG.dshHome)
        return send(res, 200, { ok: true, config: cfg, categories: CATEGORIES })
      }
      if (path === '/api/backup-select' && req.method === 'POST') {
        // 保存选择配置 {session, skill, api}（profile 强制必选，不入库）
        const body = await readJson(req)
        const { saveSelectConfig } = await import('../lib/backup-select.js')
        const r = await saveSelectConfig(CFG.dshHome, {
          session: body?.session,
          skill: body?.skill,
          api: body?.api,
        })
        log('info', `备份内容选择已保存: session=${r.session} skill=${r.skill} api=${r.api} (profile 必选)`)
        return send(res, r.ok ? 200 : 400, r)
      }
      if (path === '/api/backup-select/apply' && req.method === 'POST') {
        // 按选择写 .gitignore（反向白名单）
        ...
        log(r.ok ? 'info' : 'warn', `按选择写 .gitignore: ${r.ok ? `${r.selectedCount} 个路径` : r.error}`)
        return send(res, r.ok ? 200 : 400, r)
      }
      if (path === '/api/backup-select/push' && req.method === 'POST') {
        // 按选择推送：git add 选中 → commit → push 备份仓（远端 <设备ID>/ 文件夹内）
        const { readSelectConfig, expandSelection } = await import('../lib/backup-select.js')
        const { getDeviceId } = await import('../lib/device.js')
        const cfg = await readSelectConfig(CFG.dshHome)
        const selected = expandSelection(cfg)
        // 1) git add 选中路径（-f 强制；node_modules.hoisted-bak 等靠 pushViaToken 的 onlyPaths 过滤）
        for (const sel of selected) {
          const a = await runGit(['add', '-f', sel], { cwd: CFG.dshHome })
          if (!a.ok) return send(res, 400, { ok: false, error: `git add ${sel} 失败: ${a.stderr || a.error}` })
        }
        // 2) commit
        const commitMsg = `chore(backup-select): 备份 ${selected.join(',')} @ ${new Date().toISOString().slice(0, 19).replace('T', ' ')}`
        const c = await runGit(['commit', '-m', commitMsg], { cwd: CFG.dshHome })
        if (!c.ok && !/nothing to commit|no changes/i.test(c.stderr || '')) {
          return send(res, 400, { ok: false, error: `commit 失败: ${c.stderr || c.error}` })
        }
        // 3) token 快照推送（远端 <设备ID>/ 前缀 + onlyPaths 过滤，2026-08-21）
        const { pushSnapshot } = await import('../lib/github.js')
        const { defaultBackupRepo } = await import('../lib/device.js')
        const token = await readTokenForUpdate()
        const backup = await defaultBackupRepo(join(CFG.dshHome, 'git-rescue'))
        const { id } = await getDeviceId(join(CFG.dshHome, 'git-rescue'))
        const r = await pushSnapshot({
          token: token || '',
          owner: 'EIGHTfs',
          repo: backup.repo,
          repoDir: CFG.dshHome,
          pathPrefix: id.slice(0, 12),
          onlyPaths: selected,
          message: commitMsg,
        })
        log(r.ok ? 'info' : 'warn', `按选择推送: ${r.ok ? `${r.files} 文件 → ${r.repo}/${id.slice(0, 12)}/` : r.error}`)
        return send(res, r.ok ? 200 : 500, { ok: r.ok, ...r, deviceDir: id.slice(0, 12) })
      }
      if (path === '/api/gitlog') {
        const n = Number(url.searchParams.get('n') || 30)
        // 结构化提交列表：hash / 短hash / message / 是否 bad 标记 / 是否当前 HEAD
        const logR = await runGit(['log', '--pretty=format:%H%x09%s', '-n', String(n)], { cwd: CFG.dshHome })
        const { listBadTags } = await import('../lib/git.js')
        const badTags = await listBadTags(CFG.dshHome)
        const badShorts = new Set(badTags.map((t) => t.replace('bad-', '').slice(0, 8)))
        const headR = await runGit(['rev-parse', 'HEAD'], { cwd: CFG.dshHome })
        const headShort = headR.ok ? headR.stdout.trim().slice(0, 8) : ''
        const commits = (logR.ok ? logR.stdout.split('\n') : []).filter(Boolean).map((line) => {
          ...
          return {
            ...
            bad: badShorts.has(short),
            current: short === headShort,
          }
        })
        // 可回退点：非 bad、非当前 HEAD 的提交（bad 标记的不可选）
        const rollbackPoints = commits.filter((c) => !c.bad && !c.current)
        return send(res, 200, { ok: true, head: headShort, commits, rollbackPoints, badCount: badTags.length })
      }
      if (path === '/api/gitlog/rollback' && req.method === 'POST') {
        // 回退到指定提交（2026-08-21）：commit 现场 → 标记 bad → reset --hard 到目标 ref
        const body = await readJson(req).catch(() => ({}))
        const ref = String(body?.ref || '').trim()
        if (!/^[0-9a-f]{7,40}$/i.test(ref)) return send(res, 400, { ok: false, error: 'ref 无效（需 7-40 位 hex）' })
        const { hardReset, markBad } = await import('../lib/git.js')
        // 1) 先 commit 当前现场（坏现场留痕）
        const before = await commit(CFG.dshHome, `chore(rollback): 回退前现场 @ ${new Date().toISOString()}`)
        // 2) 给刚生成的提交（或当前 HEAD）打 bad 标记
        const cur = await runGit(['rev-parse', 'HEAD'], { cwd: CFG.dshHome })
        if (cur.ok) await markBad(CFG.dshHome, cur.stdout.trim())
        // 3) reset --hard 到目标
        const r = await hardReset(CFG.dshHome, ref)
        if (!r.ok) return send(res, 500, { ok: false, error: r.error || '回退失败' })
        log('info', `↩ 手动回退到 ${ref.slice(0, 8)}（现场已留痕 bad-${(cur.ok ? cur.stdout.trim() : '').slice(0, 8)}）`)
        return send(res, 200, { ok: true, to: ref.slice(0, 8), badTag: cur.ok ? `bad-${cur.stdout.trim().slice(0, 8)}` : null })
      }
      // ===== 远端备份库入口（P2-2，2026-08-21）=====
      if (path === '/api/backup/status' && req.method === 'GET') {
        const { defaultBackupRepo, getDeviceId, getDshVersion } = await import('../lib/device.js')
        const auth = await authStatus()
        const backup = await defaultBackupRepo(join(CFG.dshHome, 'git-rescue'))
        const dshVersion = await getDshVersion()
        // 读取最近推送事件（插件 events.jsonl）
        const eventsPath = join(CFG.dshHome, 'git-rescue', 'events.jsonl')
        let lastPush = null
        try {
          const lines = (await fs.readFile(eventsPath, 'utf8')).split('\n').filter(Boolean)
          for (let i = lines.length - 1; i >= 0; i--) {
            const evt = JSON.parse(lines[i])
            if (evt.type === 'push') { lastPush = evt; break }
          }
        } catch { /* 无事件文件 */ }
        return send(res, 200, {
          ok: true,
          ...
          lastPush: lastPush ? {
            time: lastPush.ts || '',
            commit: (lastPush.commit || '').slice(0, 8),
            files: lastPush.files || 0,
            method: lastPush.method || 'unknown',
            error: lastPush.error || null,
          } : null,
        })
      }
      if (path === '/api/backup/push' && req.method === 'POST') {
        const { pushSnapshot } = await import('../lib/github.js')
        const { defaultBackupRepo, getDshVersion } = await import('../lib/device.js')
        const token = await readTokenForUpdate()
        // 仅 token 方案（2026-08-21 用户决定：删除 SSH key，只走 token）
        const backup = await defaultBackupRepo(join(CFG.dshHome, 'git-rescue'))
        const dshVersion = await getDshVersion()
        // 支持请求体覆盖 repo 名称（用于手动指定已有仓库）
        const body = await readJson(req).catch(() => ({}))
        const repo = body?.repo || backup.repo
        const r = await pushSnapshot({
          token: token || '',
          owner: 'EIGHTfs',
          repo,
          repoDir: CFG.dshHome,
          message: `dsh-git-rescue backup @ ${new Date().toISOString()}`,
        })
        if (r.ok) log('info', `📤 远端备份推送成功: ${r.repo} (${r.files} 文件, ${r.method})`)
        else log('warn', `📤 远端备份推送失败: ${r.error}`)
        return send(res, r.ok ? 200 : 500, { ok: r.ok, ...r })
      }
      if (path === '/api/backup/fetch' && req.method === 'GET') {
        // 预览远端备份（列出 <设备ID>/ 下文件，不写入）
        const { fetchBackupSnapshot } = await import('../lib/github.js')
        const { defaultBackupRepo, getDeviceId } = await import('../lib/device.js')
        const token = await readTokenForUpdate()
        const backup = await defaultBackupRepo(join(CFG.dshHome, 'git-rescue'))
        const { id } = await getDeviceId(join(CFG.dshHome, 'git-rescue'))
        const r = await fetchBackupSnapshot({
          token: token || '',
          owner: 'EIGHTfs',
          repo: backup.repo,
        ...
        if (r.ok) log('info', `🔍 远端备份预览: ${r.files} 个文件（${backup.repo}/${id.slice(0, 12)}/）`)
        else log('warn', `🔍 远端备份预览失败: ${r.error}`)
        return send(res, r.ok ? 200 : 500, { ok: r.ok, repo: backup.repo, deviceId: id.slice(0, 12), files: r.files, list: r.restored, error: r.error })
      }
      if (path === '/api/backup/fetch' && req.method === 'POST') {
        // 从远端备份库拉取恢复（写入 .dsh）
        const { fetchBackupSnapshot } = await import('../lib/github.js')
        const { defaultBackupRepo, getDeviceId } = await import('../lib/device.js')
        const token = await readTokenForUpdate()
        ...
        const r = await fetchBackupSnapshot({
          token: token || '',
          owner: 'EIGHTfs',
          repo: body?.repo || backup.repo,
          deviceId: body?.deviceId || id.slice(0, 12),
          destDir: CFG.dshHome,
        })
        if (r.ok) log('info', `📥 从远端拉取恢复: ${r.files} 个文件写入 .dsh`)
        else log('warn', `📥 从远端拉取恢复失败: ${r.error}`)
        return send(res, r.ok ? 200 : 500, { ok: r.ok, ...r })
      }
      // ===== 守护进程重启方案（web 多选共同启用，2026-08-21）=====
      if (path === '/api/protection' && req.method === 'GET') {
        const { protectionStatus } = await import('../lib/protection.js')
        const r = await protectionStatus(CFG.dshHome)
        return send(res, 200, r)
      }
      if (path === '/api/protection' && req.method === 'POST') {
        // 保存多选方案并应用到差异（enable 新增 / disable 移除）
        const body = await readJson(req).catch(() => ({}))
        const { readProtectionConfig, saveProtectionConfig, enableMethod, disableMethod, methodAvailable } = await import('../lib/protection.js')
        const requested = Array.isArray(body?.methods) ? body.methods : []
        const before = await readProtectionConfig(CFG.dshHome)
        const after = await saveProtectionConfig(CFG.dshHome, requested)
        const results = []
        // 启用新增的（不可用方案：保留勾选，标记 pending，可用时再生效）
        for (const m of after.methods) {
          if (!before.methods.includes(m)) {
            if (!methodAvailable(m)) {
              results.push({ id: m, action: 'enable', ok: true, pending: true, error: '当前环境不可用，勾选已保留，可用时自动生效' })
              log('info', `🛡️ 守护方案勾选 [${m}]: 当前不可用，标记待生效`)
              continue
            }
            const r = await enableMethod(m)
            results.push({ id: m, action: 'enable', ok: r.ok, error: r.error, pending: false })
            log(r.ok ? 'info' : 'warn', `🛡️ 守护方案启用 [${m}]: ${r.ok ? 'ok' : r.error}`)
          }
        }
        // 停用移除的
        for (const m of before.methods) {
          if (!after.methods.includes(m)) {
            const r = await disableMethod(m)
            results.push({ id: m, action: 'disable', ok: r.ok, error: r.error })
            log(r.ok ? 'info' : 'warn', `🛡️ 守护方案停用 [${m}]: ${r.ok ? 'ok' : r.error}`)
          }
        }
        return send(res, 200, { ok: true, config: after, results })
      }
      ...
      // ===== 救援环境安装器（2026-08-21：主环境只装小插件，专职更新救援环境）=====
      if (path === '/api/installer/status' && req.method === 'GET') {
        const { installerStatus } = await import('../lib/installer.js')
        const { getDshVersion } = await import('../lib/device.js')
        const version = (await getDshVersion()) || '0.1.0-rc.6'
        const r = await installerStatus(version)
        return send(res, 200, r)
      }
      if (path === '/api/installer/install' && req.method === 'POST') {
        const body = await readJson(req).catch(() => ({}))
        const { installLatestToRescueEnvs } = await import('../lib/installer.js')
        const { getDshVersion } = await import('../lib/device.js')
        const version = (await getDshVersion()) || '0.1.0-rc.6'
        const targets = Array.isArray(body?.targets) ? body.targets : ['test', 'clean']
        log('info', `🛠 救援环境安装器：拉取最新源码安装到 ${targets.join('+')}（dsh ${version}）`)
        const r = await installLatestToRescueEnvs({ dshVersion: version, targets })
        if (r.ok) log('info', `🛠 安装器完成: v${r.version} → ${r.results.map((x) => `${x.target}:${x.ok ? 'ok' : x.error}`).join(' | ')}`)
        else log('warn', `🛠 安装器失败: ${r.error}`)
        return send(res, r.ok ? 200 : 500, r)
      }
