...
| # | 故障根因 | 代码位置 | 触发方式 | 验证 |
|---|----------|----------|----------|------|
| 1 | **OOM 崩溃**（数据写坏→启动吃满 Node 默认 2GB heap→SIGABRT） | `guardian/server.js` `startDsh()`：spawn 时 `env.NODE_OPTIONS = computeMaxOldSpace()`（`lib/fault-classify.js`：物理内存 50%、下限 2048/上限 8192，env `DSH_MAX_OLD_SPACE` 可覆盖） | guardian 拉起 DSH 时自动带 | 启动后 60s 不崩 = 生效；`tr '\0' '\n' < /proc/<pid>/environ \| grep NODE_OPTIONS` |
| 1b | **OOM 崩溃误走 git 回退**（2026-08-21 新增：用户实测"内存崩了好几次"——OOM 被判 unknown→回退无用→拉起→又崩死循环） | `lib/fault-classify.js` `classifyFault()` 新增 `oom` 类型（heap OOM / FATAL ERROR / SIGABRT / oom-killer / Out of memory）+ `guardian/server.js` `recover()` OOM 分支（`faultInfo.type==='oom'`：跳过 git 回退/坏点标记，记录内存诊断后直接拉起，OOM 崩溃后进程已退内存已释放拉起成功率最高）+ `tick()` `!recoverable` 分支 oom 特判 | guardian 故障分类自动命中 | 单测 T16（12 断言）；status 暴露 `mem`/`oomProtection` |
...
> ✅ 2026-08-21：7 项已代码化（1/1b/2/3/4/5/6），无"仅文档待补"项。
